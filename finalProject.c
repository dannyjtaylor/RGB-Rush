...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : FreeRTOS Game: Final Logic + Detailed Debugging
  ******************************************************************************
  */
...
// Structure to pass data via Queue
typedef struct {
    Color p1;
    Color p2;
    uint16_t s1_raw; // Added Raw ADC for debugging
    uint16_t s2_raw;
} InputData_t;
/* USER CODE END PTD */
...
// Digit Selects
#define DIG1_COM_Pin GPIO_PIN_7
#define DIG1_COM_Port GPIOC
#define DIG2_COM_Pin GPIO_PIN_9
#define DIG2_COM_Port GPIOB
#define DIG3_COM_Pin GPIO_PIN_8
...
SemaphoreHandle_t xWinSemaphore;  // Trigger Game Over Animation
SemaphoreHandle_t xScoreMutex;

// Global State
int score = STARTING_SCORE;
int round_count = 1;
int game_active = 1; // 1 = Running, 0 = Game Over (Freeze Timer)
Color target_color;

// Debug Globals (For display task logging)
Color current_p1 = {0,0,0,"OFF"};
Color current_p2 = {0,0,0,"OFF"};
Color current_mix = {0,0,0,"OFF"};
uint16_t debug_s1 = 0;
uint16_t debug_s2 = 0;

// Display Buffers
volatile uint8_t disp_digit_1 = 0;
volatile uint8_t disp_digit_2 = 0;
volatile uint8_t disp_digit_3 = 0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM10) {
        ...
        uint8_t val = 0;
        if (current_digit_index == 0) val = disp_digit_1;
        else if (current_digit_index == 1) val = disp_digit_2;
        else val = disp_digit_3;

        Write_Segment_Bus(val);
...
void InputTask(void *argument) {
    InputData_t data;
    for(;;) {
        data.s1_raw = Read_ADC1();
        data.s2_raw = Read_ADC2();
        data.p1 = Get_Color(data.s1_raw);
        data.p2 = Get_Color(data.s2_raw);

        xQueueSend(xInputQueue, &data, pdMS_TO_TICKS(10));
...
void LogicTask(void *argument) {
    ...
    for(;;) {
        if (xQueueReceive(xInputQueue, &rx, portMAX_DELAY) == pdTRUE) {

            // Update Globals for Display Task
            current_p1 = rx.p1;
            current_p2 = rx.p2;
            current_mix = Mix_Colors(current_p1, current_p2);
            debug_s1 = rx.s1_raw;
            debug_s2 = rx.s2_raw;

            // Win Logic (Only if game is active)
            if (game_active && Colors_Match(current_mix, target_color)) {
                win_hold++;
                if (win_hold >= WIN_HOLD_COUNT) {
                    ...
                    if (score == 0) {
                        // === GAME OVER CONDITION ===
                        game_active = 0; // Stop Timer
                        xSemaphoreGive(xWinSemaphore); // Trigger Animation
                    } else {
                        // === INTERMEDIATE WIN (New Round) ===
                        round_count++;

                        // SUSPEND DisplayTask so we can blink LEDs without interference
                        vTaskSuspend(displayTaskHandle);

                        // Blink Mix and Target LEDs TWICE
                        for(int i=0; i<2; i++) {
                            Set_LED_Target(OFF);
                            Set_LED_Mix(OFF);
                            ...
                            Set_LED_Mix(current_mix);
                            vTaskDelay(pdMS_TO_TICKS(200));
                        }

                        // Resume DisplayTask and Pick New Target
...
void DisplayTask(void *argument) {
    ...
    // Use FreeRTOS Tick Count instead of HAL_GetTick()
    TickType_t last_tick = xTaskGetTickCount();
    TickType_t last_log = xTaskGetTickCount();

    for(;;) {
        // 1. Check Game Over Animation
        if (xSemaphoreTake(xWinSemaphore, 0) == pdTRUE) {
            sprintf(msg, "\r\n*** GAME OVER! Final Time: %d sec ***\r\n", game_timer);
            ...
            // FREEZE STATE
            Set_LED_P1(OFF); Set_LED_P2(OFF); Set_LED_Mix(OFF); Set_LED_Target(OFF);
            disp_digit_1 = 0; // Score 0
            disp_digit_2 = game_timer / 10;
            disp_digit_3 = game_timer % 10;
        ...
        // 2. Update LEDs
        Set_LED_P1(current_p1);
        Set_LED_P2(current_p2);
        Set_LED_Mix(current_mix);
        Set_LED_Target(target_color);
        ...
        local_score = score;
        xSemaphoreGive(xScoreMutex);
        disp_digit_1 = local_score;

        // 4. Update Game Timer (Using FreeRTOS Ticks)
        if (game_active && (xTaskGetTickCount() - last_tick >= pdMS_TO_TICKS(1000))) {
            game_timer++;
            if (game_timer > 99) game_timer = 99; // Cap at 99
            last_tick = xTaskGetTickCount();
        }
        disp_digit_2 = game_timer / 10;
        disp_digit_3 = game_timer % 10;

        // 5. DEBUG LOGGING (Using FreeRTOS Ticks)
        if (xTaskGetTickCount() - last_log >= pdMS_TO_TICKS(200)) {
            sprintf(msg, "[%02d] Rnd:%d | S1:%4d(%s) S2:%4d(%s) | MIX:%s TGT:%s\r",
                    game_timer, round_count,
                    debug_s1, current_p1.name,
                    debug_s2, current_p2.name,
                    current_mix.name, target_color.name);
...
uint16_t Read_ADC1(void) {
    HAL_ADC_Start(&hadc1);
    if (HAL_ADC_PollForConversion(&hadc1, 20) == HAL_OK) return HAL_ADC_GetValue(&hadc1);
    return 0;
}
uint16_t Read_ADC2(void) {
    HAL_ADC_Start(&hadc2);
    if (HAL_ADC_PollForConversion(&hadc2, 20) == HAL_OK) return HAL_ADC_GetValue(&hadc2);
    return 0;
}
Color Get_Color(uint16_t val) {
...
void Pick_New_Target(void) {
    int r = (xTaskGetTickCount() + rand()) % 6; // Changed random seed to xTaskGetTickCount
    switch(r) {
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // Create Queue (Depth 5, Size of InputData_t)
  xInputQueue = xQueueCreate(5, sizeof(InputData_t));

  // Create Mutex and Semaphore
  xScoreMutex = xSemaphoreCreateMutex();
  xWinSemaphore = xSemaphoreCreateBinary();

  // Create Tasks (STACK INCREASED TO 512 WORDS)
  xTaskCreate(InputTask, "Input", 512, NULL, 1, &inputTaskHandle);
  xTaskCreate(LogicTask, "Logic", 512, NULL, 2, &logicTaskHandle); // Higher priority
  xTaskCreate(DisplayTask, "Disp", 512, NULL, 1, &displayTaskHandle);

  // Start Multiplexing Interrupt
...
static void MX_TIM10_Init(void)
{
  ...
  HAL_NVIC_SetPriority(TIM1_UP_TIM10_IRQn, 5, 0); // Lower prio than SysTick/FreeRTOS
